...
void draw(int x1, int y1) {
        ...
        for (int i = 0; i < 17; i++) {
            ...
            SDL_RenderDrawLine(gRenderer, SCREEN_WIDTH - x1, 24,
                               SCREEN_WIDTH - 32, SCREEN_HEIGHT - y1);

            // Draw the canvas' diagonals.

            SDL_SetRenderDrawColor(gRenderer, 0xff /*R*/, 0x00 /*G*/, 0x00 /*B*/, 0xFF /*A*/);
            SDL_RenderDrawLine(gRenderer, 32, SCREEN_HEIGHT-y1,
                               SCREEN_WIDTH-x1, SCREEN_HEIGHT-24);
            x1 += 32;
            y1 += 24;
